...
function generateAccessKey1(req, res) {
    var accessKey = createAccessKey1();
...
function generateAccessKey2(req, res) {
    var accessKey = createAccessKey2();
    saveAccessKey(accessKey);
    // Response status 200 with accessKey
    res.status(200).json({
        accessKey: accessKey
...
function generateAccessKey3(req, res) {
    var accessKey = createAccessKey3();
    saveAccessKey(accessKey);
    // Response status 200 with accessKey
    res.status(200).json({
        accessKey: accessKey
...
function createAccessKey1() {
    var apiKeyExample = config_1.acceptedAPIKeyExample1;
    var passwordExample = generateRandomPassword();
    return Buffer.from("".concat(apiKeyExample, ":").concat(passwordExample), "utf-8").toString("base64");
}
;
function createAccessKey2() {
    var apiKeyExample = config_1.acceptedAPIKeyExample2;
    var passwordExample = generateRandomPassword();
    return Buffer.from("".concat(apiKeyExample, ":").concat(passwordExample), "utf-8").toString("base64");
}
;
function createAccessKey3() {
    var apiKeyExample = config_1.acceptedAPIKeyExample3;
    var passwordExample = generateRandomPassword();
    return Buffer.from("".concat(apiKeyExample, ":").concat(passwordExample), "utf-8").toString("base64");
}
;
...
function hasAccessKeyInDataStore(accessKey) {
    var accessKeyStore = globalThis.accessKeyStore;
    var acc = accessKeyStore.find(function (value) { return value === accessKey; });
    if (acc) {
        return true;
    }
    else {
...
function deleteAccessKey(accessKey) {
    var index = globalThis.accessKeyStore.indexOf(accessKey);
...
function validateAccessKey(req, res, next) {
    ...
    var accessKey = authentication.slice(6);
